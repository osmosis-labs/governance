...
TODO: Change the below to be osmosis commands

This is the command format for using osmosisd (the command-line interface) to submit your proposal on-chain:
```osmosisd tx gov submit-proposal <proposal type> proposal.json --from [key/address]```

If `<proposal type>` is left blank, the type will be a Text proposal. Otherwise,
it can be set to `param-change`, `community-pool-spend` or `replace-pool-incentives`. Use `--help` to get
